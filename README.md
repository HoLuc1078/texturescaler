...
# 需要 JDK 17
gradlew build
# 产物：build/libs/texturescaler-1.0.3.jar
```
...
- **v1.0.3**：继续提速——精简缺失贴图的查询路径。原图读取时若合并资源管理器（已包含全部资源包）
  查不到某贴图，此前还会再按资源包逐个查一遍（每次重载缺失查询多达 3.7 万次，等于把同样的查找
  重复两遍）；现在合并管理器查不到即判定缺失。兜底遍历仅保留给重入保护路径和异常场景。
